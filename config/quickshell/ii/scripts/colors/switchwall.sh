...
# Static images only; requires swww-daemon when background.wallpaperDaemon is "swww".
apply_swww_if_configured() {
    local path="$1"
    [[ -z "$path" || ! -f "$path" ]] && return 0
    [[ ! -f "$SHELL_CONFIG_FILE" ]] && return 0
    local daemon
    daemon=$(jq -r '.background.wallpaperDaemon // "quickshell"' "$SHELL_CONFIG_FILE")
    [[ "$daemon" != "swww" ]] && return 0
    if ! command -v swww &>/dev/null; then
        notify-send -a "Wallpaper switcher" -c "im.error" \
            "swww not found" \
            "Install swww, or set background.wallpaperDaemon to quickshell in illogical-impulse config."
        return 0
    fi
    local abs
    abs=$(readlink -f "$path" 2>/dev/null || realpath "$path" 2>/dev/null || echo "$path")
    local t_type t_dur t_fps filt
    t_type=$(jq -r '.background.swwwTransitionType // "simple"' "$SHELL_CONFIG_FILE")
    t_dur=$(jq -r '.background.swwwTransitionDuration // 2' "$SHELL_CONFIG_FILE")
    t_fps=$(jq -r '.background.swwwTransitionFps // 60' "$SHELL_CONFIG_FILE")
    filt=$(jq -r '.background.swwwFilter // "Lanczos3"' "$SHELL_CONFIG_FILE")
    local args=(img --resize crop -f "$filt" --transition-type "$t_type" --transition-fps "$t_fps")
    if [[ "$t_type" != "simple" && "$t_type" != "none" ]]; then
        args+=(--transition-duration "$t_dur")
    fi
    args+=("$abs")
    if ! swww "${args[@]}"; then
        notify-send -a "Wallpaper switcher" -c "im.error" \
            "swww failed" \
            "Is swww-daemon running? Uncomment exec-once in hypr execs.conf or start it manually."
    fi
}
...
switch() {
    ...
    matugen_args=()
    sci=$(jq -r '(.appearance.wallpaperTheming.matugen // {}) | .sourceColorIndex // 0' "$SHELL_CONFIG_FILE")
    matugen_args+=(--source-color-index "$sci")
    contrast=$(jq -r '(.appearance.wallpaperTheming.matugen // {}) | .contrast // 0' "$SHELL_CONFIG_FILE")
    matugen_args+=(--contrast "$contrast")
    prefer=$(jq -r '(.appearance.wallpaperTheming.matugen // {}) | .prefer // ""' "$SHELL_CONFIG_FILE")
    if [[ -n "$prefer" && "$prefer" != "null" ]]; then
        matugen_args+=(--prefer "$prefer")
    fi
    rf=$(jq -r '(.appearance.wallpaperTheming.matugen // {}) | .resizeFilter // "lanczos3"' "$SHELL_CONFIG_FILE")
    if [[ -n "$rf" && "$rf" != "null" ]]; then
        matugen_args+=(--resize-filter "$rf")
    fi
    ld=$(jq -r '(.appearance.wallpaperTheming.matugen // {}) | .lightnessDark // 0' "$SHELL_CONFIG_FILE")
    matugen_args+=(--lightness-dark "$ld")
    ll=$(jq -r '(.appearance.wallpaperTheming.matugen // {}) | .lightnessLight // 0' "$SHELL_CONFIG_FILE")
    matugen_args+=(--lightness-light "$ll")
    inc=$(jq -r '(.appearance.wallpaperTheming.matugen // {}) | .includeImageInJson // false' "$SHELL_CONFIG_FILE")
    if [[ "$inc" == "true" ]]; then
        matugen_args+=(--include-image-in-json true)
    fi
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if is_video "$imgpath"; then
            ...
            set_wallpaper_path "$imgpath"
            remove_restore
            apply_swww_if_configured "$imgpath"
        fi
    fi
