...
Singleton {
    ...
    /**
     * Desktop clock quote: build displayed line from body + Config quote object (prefix/suffix, quotes, whitespace).
     * @param { string } rawBody
     * @param { object } q — Config.options.background.widgets.clock.quote
     * @returns { string }
     */
    function formatClockQuoteLine(rawBody, q) {
        if (!q)
            return rawBody ?? "";
        let body = String(rawBody ?? "");
        if (q.collapseWhitespace === true)
            body = body.replace(/\s+/g, " ").trim();
        else
            body = body.trim();

        if (q.trimOuterQuotes === true) {
            const openQ = new Set(['"', "\u201C", "\u201D", "\u201E", "\u00AB"]);
            const closeQ = new Set(['"', "\u201C", "\u201D", "\u201E", "\u00BB"]);
            while (body.length >= 2 && openQ.has(body[0]) && closeQ.has(body[body.length - 1])) {
                body = body.slice(1, -1).trim();
            }
        }

        if (q.useAsciiQuotes === true) {
            body = body.replace(/\u201C|\u201D|\u201E/g, '"').replace(/\u2018|\u2019/g, "'");
        }

        let t = q.uppercase === true ? body.toUpperCase() : body;
        let pre = String(q.prefix ?? "");
        let suf = String(q.suffix ?? "");
        if (q.useAsciiQuotes === true) {
            pre = pre.replace(/\u201C|\u201D|\u201E/g, '"').replace(/\u2018|\u2019/g, "'");
            suf = suf.replace(/\u201C|\u201D|\u201E/g, '"').replace(/\u2018|\u2019/g, "'");
        }

        if (q.autoWrapQuotes === true && pre.length === 0 && suf.length === 0) {
            if (q.useAsciiQuotes === true) {
                pre = "\"";
                suf = "\"";
            } else {
