...
AbstractBackgroundWidget {
    ...
    readonly property string clockStyle: GlobalStates.screenLocked ? (Config.options.background.widgets.clock.cliStyle ? "digital" : Config.options.background.widgets.clock.styleLocked) : (Config.options.background.widgets.clock.cliStyle ? "digital" : Config.options.background.widgets.clock.style)
    ...
    readonly property bool quoteHasContent: {
        ...
        for (let i = 0; i < pool.length; ++i) {
            if (String(pool[i] ?? "").trim().length > 0)
                return true;
