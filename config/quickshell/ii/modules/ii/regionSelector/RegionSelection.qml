...
PanelWindow {
    ...
    property bool hideBottomChromeForSelection: false

    onVisibleChanged: {
        if (!visible) {
            hideBottomChromeForSelection = false;
            regionSelectionControls.anchors.bottomMargin = -regionSelectionControls.height;
            regionSelectionControls._enteredOpacity = 0;
        }
    }
    ...
    function setRegionToTargeted() {
        ...
        root.selTargetX = root.regionX;
        root.selTargetY = root.regionY;
        root.selTargetW = root.regionWidth;
        root.selTargetH = root.regionHeight;
    }
    ...
    property real selTargetX: 0
    property real selTargetY: 0
    property real selTargetW: 0
    property real selTargetH: 0

    readonly property bool smoothRectDragging: root.dragging && root.selectionMode === RegionSelection.SelectionMode.RectCorners && Config.options.regionSelector.rect.smoothSelection

    Behavior on regionX {
        enabled: root.smoothRectDragging
        SpringAnimation {
            spring: 4
            damping: 0.4
        }
    ...
    Timer {
        id: smoothRectSelectionTimer
        interval: Math.max(1, Config.options.regionSelector.rect.smoothSelectionIntervalMs)
        repeat: true
        running: root.smoothRectDragging
        onTriggered: {
            root.regionX = root.selTargetX;
            root.regionY = root.selTargetY;
            root.regionWidth = root.selTargetW;
            root.regionHeight = root.selTargetH;
        }
    }
    ...
    MouseArea {
        ...
        // Controls
        onPressed: (mouse) => {
            root.hideBottomChromeForSelection = true;
            ...
            root.selTargetX = mouse.x;
            root.selTargetY = mouse.y;
            root.selTargetW = 0;
            root.selTargetH = 0;
            root.regionX = mouse.x;
            root.regionY = mouse.y;
            root.regionWidth = 0;
            root.regionHeight = 0;
        }
        onReleased: (mouse) => {
            const clickSelection = root.draggingX === root.dragStartX && root.draggingY === root.dragStartY;
            root.dragging = false;
            ...
            } else if (root.selectionMode === RegionSelection.SelectionMode.RectCorners && Config.options.regionSelector.rect.smoothSelection) {
                root.regionX = root.selTargetX;
                root.regionY = root.selTargetY;
                root.regionWidth = root.selTargetW;
                root.regionHeight = root.selTargetH;
        ...
        onPositionChanged: (mouse) => {
            ...
            if (root.selectionMode === RegionSelection.SelectionMode.Circle) {
                ...
                root.regionWidth = Math.abs(mouse.x - root.dragStartX);
                root.regionHeight = Math.abs(mouse.y - root.dragStartY);
            } else {
                root.selTargetX = Math.min(root.dragStartX, mouse.x);
                root.selTargetY = Math.min(root.dragStartY, mouse.y);
                root.selTargetW = Math.abs(mouse.x - root.dragStartX);
                root.selTargetH = Math.abs(mouse.y - root.dragStartY);
                if (!Config.options.regionSelector.rect.smoothSelection) {
                    root.regionX = root.selTargetX;
                    root.regionY = root.selTargetY;
                    root.regionWidth = root.selTargetW;
                    root.regionHeight = root.selTargetH;
        ...
        Row {
            ...
            z: 10
            visible: root.phase === RegionSelection.Phase.Select
            enabled: !root.hideBottomChromeForSelection
            property real _enteredOpacity: 0
            opacity: root.hideBottomChromeForSelection ? 0 : _enteredOpacity
            anchors {
            ...
            Connections {
                ...
                function onVisibleChanged() {
                    ...
                    regionSelectionControls.anchors.bottomMargin = 28;
                    regionSelectionControls._enteredOpacity = 1;
                }
            }
            Behavior on opacity {
